...
% TODO: Not happy about the fact, that we only have one subsection here.
%       We should either have two or remove this title.
...
\cite{HosseinzadehLLP18} that was supposed to withstand the SGX-Step attack.
...
\begin{figure}[t]
  ...
  \caption{Overview of our simulated cache attack.
    After interrupting the enclave \ding{172} and resuming it \ding{173},
    control is transferred to the exception handler due to AEX-Notify.
    The exception handler will increment a shared variable \ding{174}
    and resume the enclave using \textsc{edeccssa} \ding{175}.
    Our spy thread monitors this variable until it changes and reconfigures
    the APIC timer \ding{176} to interrupt the enclave a second time \ding{177}.}
  \label{fig:cache-attack}
\end{figure}
...
An overview of our attack is illustrated in Figure~\ref{fig:cache-attack}.
