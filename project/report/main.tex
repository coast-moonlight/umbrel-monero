...
\todo{We should include a figure for square-and-multiply. It should be based on
  RSA as the text is talking about cipher text, keys and output. It should also
  include a square function and addition function such that is clear that these
  are separate function on different pages.}
...
reports the instruction pointer after each interrupt.
If the instruction pointer is incremented by one instruction each interrupt,
then we know that we are single-stepping the enclave.
